...
#if defined(__GNUC__) && defined(__GNUC_VERSION__) && (__GNUC_VERSION__ >= 406)
#define XRC_DISABLE_GCC_WARNING(w) _Pragma("GCC diagnostic push") _Pragma(XDGW3(w))
#elif defined(__GNUC__) && defined(__GNUC_VERSION__) && \
    (__GNUC_VERSION__ >= 404)  // GCC 4.4 doesn't support diagnostic push, but supports disabling warnings.
...
#if defined(__GNUC__) && defined(__GNUC_VERSION__) && (__GNUC_VERSION__ >= 4006)
...
/// and the fourth being four bytes. This is useful for exercising a runtime's requirement
...
#define XRC_UTF8_INVALID_EXERCISE_STR "\x61\xC8\xE5\x86\x98"
    ...
    public:
        RandEngine();
        explicit RandEngine(uint64_t seed);
    ...
    public:
        ...
        explicit StringVec(std::vector<std::string> const& other);
        ...
        // clang-format on
        // suppression required because clang-format 16 and 11 disagree on spacing before the &
        {
            return strPtrVector;
        }
