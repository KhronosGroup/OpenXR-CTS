...
If you are not using one of the pre-built CTS binaries created with the
CTS release, then when you plan to submit for conformance, please observe
the following considerations to ensure that the build system has accurate
source code revision information available to embed in the test suite and
output reports.
