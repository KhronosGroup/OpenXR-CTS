...
namespace Conformance
{
    ...
    TEST_CASE("XR_KHR_extended_struct_name_lengths", "[XR_KHR_extended_struct_name_lengths]")
    {
        GlobalData& globalData = GetGlobalData();
        if (!globalData.IsInstanceExtensionSupported(XR_KHR_EXTENDED_STRUCT_NAME_LENGTHS_EXTENSION_NAME)) {
            SKIP(XR_KHR_EXTENDED_STRUCT_NAME_LENGTHS_EXTENSION_NAME " not supported");
        }

        AutoBasicInstance instance({XR_KHR_EXTENDED_STRUCT_NAME_LENGTHS_EXTENSION_NAME});

        auto xrStructureTypeToString2KHR =
            GetInstanceExtensionFunction<PFN_xrStructureTypeToString2KHR>(instance, "xrStructureTypeToString2KHR");
        ...
        // valid structure types
        {
            ...
            for (auto value : structureTypeStringMap) {
                result = xrStructureTypeToString2KHR(instance, value.first, buffer);
        ...
        // exercise XR_UNKNOWN_STRUCTURE_TYPE_XXX
        {
            ...
            result = xrStructureTypeToString2KHR(instance, static_cast<XrStructureType>(UnknownType), buffer);
            REQUIRE(ValidateResultAllowed("xrStructureTypeToString2KHR", result));
            REQUIRE(result == XR_SUCCESS);
        ...
        OPTIONAL_INVALID_HANDLE_VALIDATION_SECTION
        {
            ...
            {
                result = xrStructureTypeToString2KHR(XR_NULL_HANDLE_CPP, XR_TYPE_UNKNOWN, buffer);
                REQUIRE(ValidateResultAllowed("xrStructureTypeToString2KHR", result));
