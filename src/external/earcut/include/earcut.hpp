...
void Earcut<N>::indexSegment(Node* head, Node* stop) {
    ...
    do {
        ...
        do {
            ...
            if (c->x < localMinX) localMinX = c->x;
            if (c->x > localMaxX) localMaxX = c->x;
            if (c->y < localMinY) localMinY = c->y;
            if (c->y > localMaxY) localMaxY = c->y;
            p = c;
        } while (++k < K && p != stop);
        blockStop[b] = p;
        const std::size_t g = b * 4;
        blockBBox[g] = localMinX;
        blockBBox[g + 1] = localMinY;
        blockBBox[g + 2] = localMaxX;
        blockBBox[g + 3] = localMaxY;
    } while (p != stop);
}
...
bool Earcut<N>::intersectsPolygon(const Node* a, const Node* b) {
    ...
    const double localMinX = std::min<double>(a->x, b->x);
    const double localMaxX = std::max<double>(a->x, b->x);
    const double localMinY = std::min<double>(a->y, b->y);
    const double localMaxY = std::max<double>(a->y, b->y);

    const Node* p = a;
    do {
        const Node* n = p->next;
        if ((p->x > localMaxX && n->x > localMaxX) || (p->x < localMinX && n->x < localMinX) || (p->y > localMaxY && n->y > localMaxY) ||
            (p->y < localMinY && n->y < localMinY)) {
            p = n;
            continue;
        }
